...
cnt = 0
sql = cur.execute("SELECT URL FROM url_education") #edit categories: education, entertainment, it, sports
url = cur.fetchall()

def store(URL,title,genre,body,time):
    cur.execute("INSERT INTO article(URL,title,genre,body,time) VALUES(\"%s\",\"%s\",\"%s\",\"%s\",\"%s\")",(URL,title,genre,body,time))
...
if __name__ == '__main__':
    for i in url:
        getLinks(i[0],"education") #edit categories: education, entertainment, it, sports
